<template>
  <div class="cm-container" :style="axisComputed" v-if="show">
    ...
    >
      <symbol id="icon-children-arrow" viewBox="0 0 1024 1024">
        <path
    ...
    <ul class="cm-ul cm-ul-1">
      <li v-for="(item, index) in list" :key="index" :style="liStyle">
        <div @click.stop="callback(item)" :class="firstLeft ? 'cm-left' : ''">
          <i :class="item.icon"></i>
          <span>{{ item.text }}</span>
          <svg
            class="icon"
          ...
          >
            <use xlink:href="#icon-children-arrow"></use>
          </svg>
        </div>
        <!--second-->
        <ul
        ...
        >
          <li v-for="(second, si) in item.children" :key="si" :style="liStyle">
            <div
              @click.stop="callback(second)"
...
export default {
  ...
  methods: {
    ...
    callback(indexList) {
      this.$emit("rd-cm-click", indexList);
    },
    hide() {
      this.show = false;
    },
